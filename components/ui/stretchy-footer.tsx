  ...
  const bars = useMemo(() => {
    ...
    return Array.from({ length: count }, (_, i) => {
      ...
      return {
        key: i,
        height: `${heightPct}%`,
        backgroundImage: `linear-gradient(to top, ${hexToRgba(color, 1)} 0%, ${hexToRgba(color, 0.6)} 45%, ${hexToRgba(color, 0)} 100%)`,
      }
    })
  }, [colors, columns])
  ...
  useEffect(() => {
    ...
    const prevOverscroll =
      bound.kind === "window"
        ...
        : bound.el.style.overscrollBehaviorY
    if (bound.kind === "window") {
      document.documentElement.style.overscrollBehaviorY = "none"
    } else {
      bound.el.style.overscrollBehaviorY = "none"
    }

    const isAtBottom = () =>
      bound.kind === "window" ? windowAtBottom() : elementAtBottom(bound.el)
    ...
    return () => {
      ...
      if (bound.kind === "window") {
        ...
        bound.el.style.overscrollBehaviorY = prevOverscroll
        const el = bound.el
    ...
    >
      ...
      >
        ...
        <div
          ...
          style={{
            backgroundImage:
              `radial-gradient(ellipse 70% 100% at 50% 100%, ${hexToRgba(colors[0] ?? "#f8f8f6", 0.22)}, ${hexToRgba(colors[0] ?? "#f8f8f6", 0)} 70%)`,
          }}
        />
      </motion.div>
    </div>
